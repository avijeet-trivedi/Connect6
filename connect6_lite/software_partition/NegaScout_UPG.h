...
#ifndef NegaScout_UPG_h
#define NegaScout_UPG_h

int leaf_node_eval (int row1, int col1, int row2, int col2, bool turn){
    ...
    if(itr0!=tt0.end()){
        ...
        Node_count++ ;
        leaf_nodes++;
        int BestVal = send_move(row1, col1, row2, col2, turn);
        tt0[key] = BestVal;
        return BestVal;
    }
}
    ...
    if(depth>1){
        while (move_count!=0){
            ...
            if (moveVal>BestVal){
                ...
                //else BestVal = -NegaMax(board, depth-1, -beta, -moveVal, Nxturn);
                else BestVal = -NegaScout(board, depth-1, -beta, -moveVal, Nxturn);   // Change AKT : Compared with Negascout
            }
        ...
        while (move_count!=0){
            ...
            moveVal= -leaf_node_eval(m.row[0], m.col[0], m.row[1], m.col[1], false);
