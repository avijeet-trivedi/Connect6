...
#ifndef ThreatFinder_h
#define ThreatFinder_h

void threat_finder(char board[19][19], player turn){

//	XTime_GetTime(&t_elapse);
//	auto start =  1.0 * ((t_elapse) / (COUNTS_PER_SECOND/1000000));
    ...
    // Horizontal Sections
    char  section_p_hs[num_hs][len_hs] ;
    char  section_o_hs[num_hs][len_hs] ;
    
    // Vertical Sections
    char  section_p_vs[num_vs][len_vs] ;
    char  section_o_vs[num_vs][len_vs] ;
    
    // Diagonal Sections (135 degree)
    char  section_p_rev_diag[num_diag][min(len_vs,len_hs)] ;
    char  section_o_rev_diag[num_diag][min(len_vs,len_hs)] ;
    
    // Diagonal Sections (45 degree)
    char  section_p_diag[num_diag][min(len_vs,len_hs)] ;
    char  section_o_diag[num_diag][min(len_vs,len_hs)] ;
...
//	XTime_GetTime(&t_elapse);
//	auto stop =  1.0 * ((t_elapse) / (COUNTS_PER_SECOND/1000000));
//  auto duration = (stop - start);
//  eval_time += duration ;

}

#endif /* ThreatFinder_h */
